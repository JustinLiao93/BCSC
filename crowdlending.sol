// acclaim the version of solidity
...
contract CrowdLending {
    ...
    function hasActiveLoan(address borrower) public constant returns(bool) {
    ...
    function newLoan(uint amount, uint dueDate, bytes32 collateral) public {
    ...
    function newProposal(uint loanId, uint rate) payable public {
    ...
    function repayLoan(uint loanId) payable public {
    ...
    function totalProposalsBy(address lender) public constant returns(uint) {
        return lendMap[lender].length;
    }

    function getProposalAtPosFor(address lender, uint pos) public constant returns(address, uint, ProposalState, uint, uint, uint, uint, bytes32) {
        Proposal storage prop = proposalList[lendMap[lender][pos]];
        return (prop.lender, prop.loanId, prop.state, prop.rate, prop.amount, loanList[prop.loanId].amount, loanList[prop.loanId].dueDate, loanList[prop.loanId].collateral);
    }

// BORROWER ACTIONS AVAILABLE    

    function totalLoansBy(address borrower) public  constant returns(uint) {
        return loanMap[borrower].length;
    }

    function getLoanDetailsByAddressPosition(address borrower, uint pos) public constant returns(LoanState, uint, uint, uint, uint,bytes32) {
        Loan storage obj = loanList[loanMap[borrower][pos]];
        return (obj.state, obj.dueDate, obj.amount, obj.collected, loanMap[borrower][pos], obj.collateral);
    }

    function getLastLoanState(address borrower) public constant returns(LoanState) {
    ...
    function getLastLoanDetails(address borrower) public constant returns(LoanState, uint, uint, uint, uint) {
        uint loanLength = loanMap[borrower].length;
    ...
    function getProposalDetailsByLoanIdPosition(uint loanId, uint numI) public constant  returns(ProposalState, uint, uint, uint, address) {
    ...
    function numTotalLoans() public constant returns(uint) {
